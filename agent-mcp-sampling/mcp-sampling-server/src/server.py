...
async def summarize(content: str, ctx: Context) -> str:
    """Summarize the content shared by caller"""
    _logger.info("summarize(): Started")

    # result = await ctx.sample(f"Summarize this:\n\n{content}") # result.text # v2: fastmcp
    ...
    try:
        ...
        _logger.error("summarize() error: %s", e)
        # print verbose error
        _logger.error(e, exc_info=True)
        raise
    
    _logger.info("summarize(): Completed")
