...
## Run local MCP server in docker

- Build the docker instance
```
cd mcp-server (if not already)
dockers/start.sh
```

- Test the api exposed in docker container
```
tests/consume_docker_mcp.sh
```
...
docker rmi dockers-mcp_api:latest -f
docker image prune -f
```

## Deploy the MCP server into ACA
...
```
tests/consume_containerapp_mcp.sh
```
